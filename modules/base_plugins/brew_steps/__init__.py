...
class MashStep(StepBase):
    ...
    kettle = StepProperty.Kettle("Kettle",
                                 description="Kettle in which the mashing takes place")
    ...
    def start(self):
        ...
        All method with decorator @cbpi.action("YOUR CUSTOM NAME") will be available
...
class MashInStep(StepBase):
    ...
    def execute(self):
        ...
        if self.get_kettle_temp(self.kettle) >= float(self.temp) and self.s is False:
            self.s = True
            self.notify("Step Temp Reached!", "Please press the next button to continue",
                        timeout=None)
...
class PumpStep(StepBase):
    ...
    timer = Property.Number("Timer in Minutes", configurable=True,
                            default_value=0, description="Timer is started immediately")
...
class BoilStep(StepBase):
    ...
    kettle = StepProperty.Kettle("Kettle",
                                 description="Kettle in which the boiling step takes place")
    timer = Property.Number("Timer in Minutes", configurable=True,
                            default_value=90,
