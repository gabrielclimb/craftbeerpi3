...
class KBH(FlaskView):
    ...
    def get(self):
        ...
        try:
            if not os.path.exists(self.api.app.config['UPLOAD_FOLDER'] + '/kbh.db'):
                self.api.notify(headline="File Not Found",
                                message="Please upload a Kleiner Brauhelfer Database",
                                type="danger")
                return ('', 404)
            ...
            print e
            self.api.notify(headline="Failed to load KHB database", message="ERROR",
                            type="danger")
            return ('', 500)
        finally:
            if conn:
        ...
        try:
            if request.method == 'POST':
                ...
                if file and self.allowed_file(file.filename):
                    ...
                    self.api.notify(headline="Upload Successful",
                                    message="The Kleiner Brauhelfer Database was uploaded succesfully")
                    return ('', 204)
                return ('', 404)
        except Exception as e:
            self.api.notify(headline="Upload Failed",
                            message="Failed to upload Kleiner Brauhelfer", type="danger")
    ...
    def load(self, id):
        ...
        try:
            ...
            Step.insert(**{"name": "MashIn", "type": mashinstep_type,
                           "config": {"kettle": mash_kettle, "temp": row[0]}})
            # add rest step
            for row in c.execute('SELECT * FROM Rasten WHERE SudID = ?', (id,)):
                Step.insert(**{"name": row[5], "type": mashstep_type,
                               "config": {"kettle": mash_kettle, "temp": row[3],
                                          "timer": row[4]}})
            Step.insert(**{"name": "Chil", "type": chilstep_type,
                           "config": {"timer": 15}})
            # Add cooking step
            c.execute('SELECT max(Zeit) FROM Hopfengaben WHERE SudID = ?', (id,))
            row = c.fetchone()
            Step.insert(**{"name": "Boil", "type": boilstep_type,
                           "config": {"kettle": boil_kettle,
                                      "temp": boil_temp,
                                      "timer": row[0]}})
            # Add Whirlpool step
            Step.insert(**{"name": "Whirlpool", "type": chilstep_type,
                           "config": {"timer": 15}})
