...
class BeerXMLImport(FlaskView):
    ...
    def get(self):
        if not os.path.exists(self.BEER_XML_FILE):
            self.api.notify(headline="File Not Found",
                            message="Please upload a Beer.xml File",
                            type="danger")
            return ('', 404)
        result = []
        ...
        try:
            if request.method == 'POST':
                ...
                if file and self.allowed_file(file.filename):
                    file.save(
                        os.path.join(self.api.app.config['UPLOAD_FOLDER'], "beer.xml"))
                    self.api.notify(headline="Upload Successful",
                                    message="The Beer XML file was uploaded succesfully")
                    return ('', 204)
                return ('', 404)
        except Exception as e:
            self.api.notify(headline="Upload Failed",
                            message="Failed to upload Beer xml",
                            type="danger")
            return ('', 500)
        ...
        try:
            ...
            Step.insert(**{
                ...
                "config": {
                    ...
                    "hop_1": boil_time - boil_time_alerts[0]
                    if len(boil_time_alerts) >= 1 else None,
                    "hop_2": boil_time - boil_time_alerts[1]
                    if len(boil_time_alerts) >= 2 else None,
                    "hop_3": boil_time - boil_time_alerts[2]
                    if len(boil_time_alerts) >= 3 else None,
                    "hop_4": boil_time - boil_time_alerts[3]
                    if len(boil_time_alerts) >= 4 else None,
                    "hop_5": boil_time - boil_time_alerts[4]
                    if len(boil_time_alerts) >= 5 else None
                }
            })
            # Add Whirlpool step
            Step.insert(**{"name": "Whirlpool", "type": "ChilStep",
                           "config": {"timer": 15}})
            StepView().reset()
    ...
    def getBoilAlerts(self, id):
        ...
        # to prevent multiple alerts at the same time
    ...
    def getSteps(self, id):
        ...
        for e in e.findall('./RECIPE[%s]/MASH/MASH_STEPS/MASH_STEP' % (str(id))):
            ...
            steps.append({"name": e.find("NAME").text, "temp": temp,
